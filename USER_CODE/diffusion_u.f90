...
!--       Interpolate eddy diffusivities on staggered gridpoints
          ...
          tend(k,j,i) = tend(k,j,i)                                                                &
                      ...
                      + ( kmzp * ( ( ( u(k+1,j,i+1) + u(k+1,j,i) + u(k,j,i+1) + u(k,j,i) )         &
                        ...
                        ) * 2.0_wp * ddzzw(k) * drho_air(k) * dzeta_dx_u(k,j,i) * flag             &
                      + ( kmyp * ( ( u(k,j+1,i) - u(k,j,i) ) * ddy                                 &
                      ...
                      + ( kmzp * ( ( ( u(k,j,i) + u(k,j+1,i) + u(k+1,j,i) + u(k+1,j+1,i) )         &
                        ...
                        ) * ddzzw(k) * drho_air(k) * dzeta_dy_u(k,j,i) * flag                      &
                      + ( kmzp * ( ( u(k+1,j,i) - u(k,j,i) ) * ddzzu(k+1) * dzeta_dz_u(j,i)        &
                        ...
                        ) * ddzzw(k) * drho_air(k) * dzeta_dz_u(j,i) * flag
       ENDDO
!
...
!--       Default-type surfaces, upward-facing
          ...
          DO  m = surf_s, surf_e
             ...
             tend(k,j,i) = tend(k,j,i) + ( - ( - surf_def_h(0)%usws(m) ) ) * ddzzw(k) * drho_air(k) &
                                                                           * dzeta_dz_u(j,i)
          ENDDO
!
!--       Default-type surfaces, dowward-facing (except for model-top fluxes)
          ...
          DO  m = surf_s, surf_e
             ...
             tend(k,j,i) = tend(k,j,i) + ( - surf_def_h(1)%usws(m) ) * ddzzw(k) * drho_air(k)       &
                                                                     * dzeta_dz_u(j,i)
          ENDDO
!
!--       Natural-type surfaces, upward-facing
          ...
          DO  m = surf_s, surf_e
             ...
             tend(k,j,i) = tend(k,j,i) + ( - ( - surf_lsm_h(0)%usws(m) ) ) * ddzzw(k) * drho_air(k) &
                                                                           * dzeta_dz_u(j,i)
          ENDDO
!
!--       Natural-type surfaces, downward-facing
          ...
          DO  m = surf_s, surf_e
             ...
             tend(k,j,i) = tend(k,j,i) + ( - surf_lsm_h(1)%usws(m) ) * ddzzw(k) * drho_air(k)       &
                                                                     * dzeta_dz_u(j,i)
          ENDDO
!
!--       Urban-type surfaces, upward-facing
          ...
          DO  m = surf_s, surf_e
             ...
             tend(k,j,i) = tend(k,j,i) + ( - ( - surf_usm_h(0)%usws(m) ) ) * ddzzw(k) * drho_air(k) &
                                                                           * dzeta_dz_u(j,i)
          ENDDO
!
!--       Urban-type surfaces, downward-facing
          ...
          DO  m = surf_s, surf_e
             ...
             tend(k,j,i) = tend(k,j,i) + ( - surf_usm_h(1)%usws(m) ) * ddzzw(k) * drho_air(k)       &
                                                                     * dzeta_dz_u(j,i)
          ENDDO
...
!--    Add momentum flux at model top
       IF ( use_top_fluxes  .AND.  constant_top_momentumflux )  THEN
          ...
          DO  m = surf_s, surf_e
             ...
             tend(k,j,i) = tend(k,j,i) + ( - surf_def_h(2)%usws(m) ) * ddzzw(k) * drho_air(k)       &
                                                                     * dzeta_dz_u(j,i)
          ENDDO
       ENDIF
       ! EOC - Xu
