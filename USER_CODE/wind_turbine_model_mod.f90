...
!--------------------------------------------------------------------------------------------------!
 MODULE wind_turbine_model_mod
    ...
    ! BOC - Xu
    USE arrays_3d,                                                                                 &
        ONLY:  tend, u, v, w, zu, zw, zzu, zzw, dzzw
    ! EOC - Xu
...
!--             Interpolate only if all required information is available on the current PE:
                IF ( ( ii >= nxl )  .AND.  ( ii <= nxr ) )  THEN
                   IF ( ( jj >= nys )  .AND.  ( jj <= nyn ) )  THEN
                      ...
                      ! BOC - Xu
                      eta_int = ( aa * eta_u(jj,ii)   + bb * eta_u(jj,ii+1) +                      &
                                  cc * eta_u(jj+1,ii) + dd * eta_u(jj+1,ii+1) ) / gg
                      H_int = H_av - eta_int

                      kk = ( (rbz(ring,rseg) - eta_int) /                                          &
                             (H_int - eta_int) + 0.5_wp * dzzw(1) ) / dzzw(1)
                      ! EOC - Xu
                      ...
                      ! BOC - Xu
                      u_int_1_l(inot,ring,rseg) = u_int_l + ( (rbz(ring,rseg) - eta_int) /         &
                                                              (H_int - eta_int) - zzu(kk) )        &
                                                  / dzzw(1) * ( u_int_u - u_int_l )
                      ! EOC - Xu
...
!--             Interpolate only if all required information is available on the current PE:
                IF ( ( ii >= nxl )  .AND.  ( ii <= nxr ) )  THEN
                   IF ( ( jj >= nys )  .AND.  ( jj <= nyn ) )  THEN
                      ...
                      ! BOC - Xu
                      eta_int = ( aa * eta_v(jj,ii)   + bb * eta_v(jj,ii+1) +                      &
                                  cc * eta_v(jj+1,ii) + dd * eta_v(jj+1,ii+1) ) / gg
                      H_int = H_av - eta_int
                      ...
                      ! BOC - Xu
                      v_int_1_l(inot,ring,rseg) = v_int_l + ( (rbz(ring,rseg) - eta_int) /         &
                                                              (H_int - eta_int) - zzu(kk) )        &
                                                  / dzzw(1) * ( v_int_u - v_int_l )
                      ! EOC - Xu
...
!--             Interpolate only if all required information is available on the current PE:
                IF ( ( ii >= nxl )  .AND.  ( ii <= nxr ) )  THEN
                   IF ( ( jj >= nys )  .AND.  ( jj <= nyn ) )  THEN
                      ...
                      ! BOC - Xu
                      eta_int = ( aa * eta_s(jj,ii)   + bb * eta_s(jj,ii+1) +                      &
                                  cc * eta_s(jj+1,ii) + dd * eta_s(jj+1,ii+1) ) / gg
                      H_int = H_av - eta_int
                      ...
                      ! BOC - Xu
                      w_int_1_l(inot,ring,rseg) = w_int_l + ( (rbz(ring,rseg) - eta_int) /         &
                                                              (H_int - eta_int) - zzw(kk) )        &
                                                  / dzzw(1) * ( w_int_u - w_int_l )
                      ! EOC - Xu
                   ELSE
                      w_int_1_l(inot,ring,rseg) = 0.0_wp
                   ENDIF
...
!--                      each rotor area segment:
                         ...
                         dist_u_3d = ( i * dx               - rbx(ring,rseg) )**2 +                &
                                     ...
                                     ( (k * dzzw(1) - 0.5_wp * dzzw(1)) * (H_av - eta_u(j,i))      &
                                       + eta_u(j,i) - rbz(ring,rseg) )**2

                         dist_v_3d = ( i * dx + 0.5_wp * dx - rbx(ring,rseg) )**2 +                &
                                     ( j * dy               - rby(ring,rseg) )**2 +                &
                                     ( (k * dzzw(1) - 0.5_wp * dzzw(1)) * (H_av - eta_v(j,i))      &
                                       + eta_v(j,i) - rbz(ring,rseg) )**2

                         dist_w_3d = ( i * dx + 0.5_wp * dx - rbx(ring,rseg) )**2 +                &
                                     ( j * dy + 0.5_wp * dy - rby(ring,rseg) )**2 +                &
                                     ( k * dzzw(1) * (H_av - eta_s(j,i))                           &
                                       + eta_s(j,i) - rbz(ring,rseg) )**2
                         ! EOC - Xu
!
