...
!--------------------------------------------------------------------------------------------------!
 MODULE poismg_mod
    ...
    ! BOC - Xu
    REAL(wp), DIMENSION(:,:), SAVE, ALLOCATABLE ::  f4_mg_b, f5_mg_b
    REAL(wp), DIMENSION(:,:), SAVE, ALLOCATABLE ::  f1_mg_b_, f2_mg_b_, f3_mg_b_, f4_mg_b_, f5_mg_b_
    REAL(wp), DIMENSION(:,:), SAVE, ALLOCATABLE ::  ddzzu_mg_b, ddzzw_mg_b
    REAL(wp), DIMENSION(:,:), SAVE, ALLOCATABLE ::  rho_air_mg_b, rho_air_zw_mg_b
    ! EOC - Xu
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE poismg( r )
    ...
    REAL(wp) :: tmp
    INTEGER(iwp) :: k, km1, km2, kp1, kp2, j, i, l
...
!-- Sort input arrays in even/odd blocks along k-dimension
    ...
    ! BOC - Xu
    CALL sort_k_to_even_odd_blocks( dzeta_dx_s(nzb:nzt+1,nys-1:nyn+1,nxl-1:nxr+1), grid_level )
    CALL sort_k_to_even_odd_blocks( dzeta_dx_u(nzb:nzt+1,nys-1:nyn+1,nxl-1:nxr+1), grid_level )
    CALL sort_k_to_even_odd_blocks( dzeta_dy_s(nzb:nzt+1,nys-1:nyn+1,nxl-1:nxr+1), grid_level )
    CALL sort_k_to_even_odd_blocks( dzeta_dy_v(nzb:nzt+1,nys-1:nyn+1,nxl-1:nxr+1), grid_level )
    ! EOC - Xu
...
!-- and odd indices
    DO WHILE ( residual_norm > residual_limit  .OR.  mgcycles < maximum_mgcycles )
       ...
       ! BOC - Xu
       CALL next_mg_level( d, p_loc,                                                                 &
                           dzeta_dx_s(nzb:nzt+1,nys-1:nyn+1,nxl-1:nxr+1),                            &
                           dzeta_dx_u(nzb:nzt+1,nys-1:nyn+1,nxl-1:nxr+1),                            &
                           dzeta_dy_s(nzb:nzt+1,nys-1:nyn+1,nxl-1:nxr+1),                            &
                           dzeta_dy_v(nzb:nzt+1,nys-1:nyn+1,nxl-1:nxr+1),                            &
                           dzeta_dz_s(nys-1:nyn+1,nxl-1:nxr+1),                                      &
                           p3, r)
       ! EOC - Xu
...
!-- Result has to be sorted back from even/odd blocks to sequential order
    ...
    ! BOC - Xu
    CALL sort_k_to_sequential( dzeta_dx_s(nzb:nzt+1,nys-1:nyn+1,nxl-1:nxr+1) )
    CALL sort_k_to_sequential( dzeta_dx_u(nzb:nzt+1,nys-1:nyn+1,nxl-1:nxr+1) )
    CALL sort_k_to_sequential( dzeta_dy_s(nzb:nzt+1,nys-1:nyn+1,nxl-1:nxr+1) )
    CALL sort_k_to_sequential( dzeta_dy_v(nzb:nzt+1,nys-1:nyn+1,nxl-1:nxr+1) )
    ! EOC - Xu
!
...
!--------------------------------------------------------------------------------------------------!
 ! BOC - Xu
 SUBROUTINE resid( f_mg, p_mg, dzeta_dx_s_mg, dzeta_dx_u_mg,                                       &
                   dzeta_dy_s_mg, dzeta_dy_v_mg, dzeta_dz_s_mg, r )
 ! EOC - Xu
    ...
    INTEGER(iwp) ::  km2  !< index variable along z dimension (k-2)
    INTEGER(iwp) ::  kp2  !< index variable along z dimension (k+2)
    ...
    ! BOC - Xu
    REAL(wp), DIMENSION(nzb:nzt_mg(grid_level)+1,nys_mg(grid_level)-1:nyn_mg(grid_level)+1,        &
                        nxl_mg(grid_level)-1:nxr_mg(grid_level)+1) ::  dzeta_dx_s_mg
    REAL(wp), DIMENSION(nzb:nzt_mg(grid_level)+1,nys_mg(grid_level)-1:nyn_mg(grid_level)+1,        &
                        nxl_mg(grid_level)-1:nxr_mg(grid_level)+1) ::  dzeta_dx_u_mg
    REAL(wp), DIMENSION(nzb:nzt_mg(grid_level)+1,nys_mg(grid_level)-1:nyn_mg(grid_level)+1,        &
                        nxl_mg(grid_level)-1:nxr_mg(grid_level)+1) ::  dzeta_dy_s_mg
    REAL(wp), DIMENSION(nzb:nzt_mg(grid_level)+1,nys_mg(grid_level)-1:nyn_mg(grid_level)+1,        &
                        nxl_mg(grid_level)-1:nxr_mg(grid_level)+1) ::  dzeta_dy_v_mg
    REAL(wp), DIMENSION(nys_mg(grid_level)-1:nyn_mg(grid_level)+1,                                 &
                        nxl_mg(grid_level)-1:nxr_mg(grid_level)+1) ::  dzeta_dz_s_mg
    ! EOC - Xu
...
!-- Calculate the residual
    ...
    DO  i = nxl_mg(l), nxr_mg(l)
       DO  j = nys_mg(l), nyn_mg(l)
          ...
          kp2 = ind_even_odd+2
          r(k,j,i) = f_mg(k,j,i) -                                                                        &
                        ( ddx2_mg(l) * rho_air_mg_b(k,l) * ( p_mg(k,j,i+1) + p_mg(k,j,i-1) )                &
                        + ddy2_mg(l) * rho_air_mg_b(k,l) * ( p_mg(k,j+1,i) + p_mg(k,j-1,i) )                &
                        + 0.25_wp * ddx_mg(l) * ddzzw_mg_b(k,l) * rho_air_zw_mg_b(k,l) * (                    &
                            dzeta_dx_u_mg(k,j,i+1) * ( p_mg(kp1,j,i)   + p_mg(kp1,j,i+1) + p_mg(k,j,i+1)  &
                                                  - 2.0_wp * ( p_mg(km1,j,i)   + p_mg(km1,j,i+1) ) )      &
                          - dzeta_dx_u_mg(k,j,i)   * ( p_mg(kp1,j,i-1) + p_mg(kp1,j,i)   + p_mg(k,j,i-1)  &
                                                  - 2.0_wp * ( p_mg(km1,j,i-1) + p_mg(km1,j,i)   ) )      &
                          + dzeta_dx_s_mg(k,j,i)   * ( p_mg(kp1,j,i+1) - p_mg(kp1,j,i-1)                  &
                                                  + p_mg(k,j,i+1)   - p_mg(k,j,i-1)   )                   &
                                                                     )                                    &
                        + 0.25_wp * ddy_mg(l) * ddzzw_mg_b(k,l) * rho_air_zw_mg_b(k,l) * (                    &
                            dzeta_dy_v_mg(k,j+1,i) * ( p_mg(kp1,j,i)   + p_mg(kp1,j+1,i) + p_mg(k,j+1,i)  &
                                                  - 2.0_wp * ( p_mg(km1,j,i)   + p_mg(km1,j+1,i) ) )      &
                          - dzeta_dy_v_mg(k,j,i)   * ( p_mg(kp1,j-1,i) + p_mg(kp1,j,i)   + p_mg(k,j-1,i)  &
                                                  - 2.0_wp * ( p_mg(km1,j-1,i) + p_mg(km1,j,i)   ) )      &
                          + dzeta_dy_s_mg(k,j,i)   * ( p_mg(kp1,j+1,i) - p_mg(kp1,j-1,i)                  &
                                                  + p_mg(k,j+1,i)   - p_mg(k,j-1,i)   )                   &
                                                                     )                                    &
                        + 0.0625_wp * ddzzw_mg_b(k,l) * ddzzw_mg_b(kp1,l) * rho_air_mg_b(kp1,l) * (             &
                          + dzeta_dx_s_mg(k,j,i) * dzeta_dx_u_mg(kp1,j,i) *                               &
                            ( p_mg(kp2,j,i-1) + p_mg(kp2,j,i) - p_mg(k,j,i-1) )                           &
                          + dzeta_dx_s_mg(k,j,i) * dzeta_dx_u_mg(kp1,j,i+1) *                             &
                            ( p_mg(kp2,j,i) + p_mg(kp2,j,i+1) - p_mg(k,j,i+1) )                           &
                          + dzeta_dy_s_mg(k,j,i) * dzeta_dy_v_mg(kp1,j,i) *                               &
                            ( p_mg(kp2,j-1,i) + p_mg(kp2,j,i) - p_mg(k,j-1,i) )                           &
                          + dzeta_dy_s_mg(k,j,i) * dzeta_dy_v_mg(kp1,j+1,i) *                             &
                            ( p_mg(kp2,j,i) + p_mg(kp2,j+1,i) - p_mg(k,j+1,i) )       )                   &
                        + 0.0625_wp * ddzzw_mg_b(k,l) * ddzzw_mg_b(k,l) * rho_air_zw_mg_b(k,l) * (              &
                          + dzeta_dx_s_mg(k,j,i) * dzeta_dx_u_mg(k,j,i) *                                 &
                                                  ( p_mg(kp1,j,i-1) + p_mg(kp1,j,i)   + p_mg(k,j,i-1)     &
                                                  - 2.0_wp * ( p_mg(km1,j,i-1) + p_mg(km1,j,i)   ) )      &
                          + dzeta_dx_s_mg(k,j,i) * dzeta_dx_u_mg(k,j,i+1) *                               &
                                                  ( p_mg(kp1,j,i)   + p_mg(kp1,j,i+1) + p_mg(k,j,i+1)     &
                                                  - 2.0_wp * ( p_mg(km1,j,i)   + p_mg(km1,j,i+1) ) )      &
                          + dzeta_dy_s_mg(k,j,i) * dzeta_dy_v_mg(k,j,i) *                                 &
                                                  ( p_mg(kp1,j-1,i) + p_mg(kp1,j,i)   + p_mg(k,j-1,i)     &
                                                  - 2.0_wp * ( p_mg(km1,j-1,i) + p_mg(km1,j,i)   ) )      &
                          + dzeta_dy_s_mg(k,j,i) * dzeta_dy_v_mg(k,j+1,i) *                               &
                                                  ( p_mg(kp1,j,i)   + p_mg(kp1,j+1,i) + p_mg(k,j+1,i)     &
                                                  - 2.0_wp * ( p_mg(km1,j,i)   + p_mg(km1,j+1,i) ) )      &
                                                                            )                             &
                        + dzeta_dz_s_mg(j,i) * dzeta_dz_s_mg(j,i) *                                       &
                            ddzzw_mg_b(k,l) * ddzzu_mg_b(kp1,l) * rho_air_zw_mg_b(k,l) * p_mg(kp1,j,i)          &
                        ) +                                                                               &
                        ( 2.0_wp * ( ddx2_mg(l) + ddy2_mg(l) ) * rho_air_mg_b(k,l)                          &
                        + 0.25_wp * ddx_mg(l) * ddzzw_mg_b(k,l) * rho_air_zw_mg_b(k,l) *                      &
                          ( dzeta_dx_u_mg(k,j,i) - dzeta_dx_u_mg(k,j,i+1) )                               &
                        + 0.25_wp * ddy_mg(l) * ddzzw_mg_b(k,l) * rho_air_zw_mg_b(k,l) *                      &
                          ( dzeta_dy_v_mg(k,j,i) - dzeta_dy_v_mg(k,j+1,i) )                               &
                        + 0.0625_wp * ddzzw_mg_b(k,l) * ddzzw_mg_b(kp1,l) * rho_air_mg_b(kp1,l) * (             &
                            dzeta_dx_s_mg(k,j,i) * ( dzeta_dx_u_mg(kp1,j,i) + dzeta_dx_u_mg(kp1,j,i+1) )  &
                          + dzeta_dy_s_mg(k,j,i) * ( dzeta_dy_v_mg(kp1,j,i) + dzeta_dy_v_mg(kp1,j+1,i) ) )&
                        - 0.0625_wp * ddzzw_mg_b(k,l) * ddzzw_mg_b(k,l) * rho_air_zw_mg_b(k,l)  * (             &
                            dzeta_dx_s_mg(k,j,i) * ( dzeta_dx_u_mg(k,j,i)   + dzeta_dx_u_mg(k,j,i+1)   )  &
                          + dzeta_dy_s_mg(k,j,i) * ( dzeta_dy_v_mg(k,j,i)   + dzeta_dy_v_mg(k,j+1,i)   ) )&
                        + ddzzw_mg_b(k,l) * ddzzu_mg_b(kp1,l) * rho_air_zw_mg_b(k,l) *                          &
                          ( dzeta_dz_s_mg(j,i) * dzeta_dz_s_mg(j,i) )                                     &
                        ) * p_mg(k,j,i)
          DO  k = ind_even_odd+2, nzt_mg(l)
              km1 = k-ind_even_odd-1
              kp1 = k-ind_even_odd
              km2 = k-1
              kp2 = k+1
              r(k,j,i) = f_mg(k,j,i) -                                                                    &
                        ...
                        + ddy2_mg(l) * rho_air_mg_b(k,l) * ( p_mg(k,j+1,i) + p_mg(k,j-1,i) )                &
                        + 0.25_wp * ddx_mg(l) * ddzzw_mg_b(k,l) * rho_air_zw_mg_b(k,l) * (                    &
                            dzeta_dx_u_mg(k,j,i+1) * ( p_mg(kp1,j,i)   + p_mg(kp1,j,i+1)                  &
                                                     - p_mg(km1,j,i)   - p_mg(km1,j,i+1) )                &
                          - dzeta_dx_u_mg(k,j,i)   * ( p_mg(kp1,j,i-1) + p_mg(kp1,j,i)                    &
                                                     - p_mg(km1,j,i-1) - p_mg(km1,j,i)   )                &
                          + dzeta_dx_s_mg(k,j,i)   * ( p_mg(kp1,j,i+1) - p_mg(kp1,j,i-1)                  &
                                                     + p_mg(km1,j,i-1) - p_mg(km1,j,i+1) )                &
                                                                                     )                    &
                        + 0.25_wp * ddy_mg(l) * ddzzw_mg_b(k,l) * rho_air_zw_mg_b(k,l) * (                    &
                            dzeta_dy_v_mg(k,j+1,i) * ( p_mg(kp1,j,i)   + p_mg(kp1,j+1,i)                  &
                                                     - p_mg(km1,j,i)   - p_mg(km1,j+1,i) )                &
                          - dzeta_dy_v_mg(k,j,i)   * ( p_mg(kp1,j-1,i) + p_mg(kp1,j,i)                    &
                                                     - p_mg(km1,j-1,i) - p_mg(km1,j,i)   )                &
                          + dzeta_dy_s_mg(k,j,i)   * ( p_mg(kp1,j+1,i) - p_mg(kp1,j-1,i)                  &
                                                     + p_mg(km1,j-1,i) - p_mg(km1,j+1,i) )                &
                                                                                     )                    &
                        + 0.0625_wp * ddzzw_mg_b(k,l) * ddzzw_mg_b(kp1,l) * rho_air_mg_b(kp1,l) * (             &
                          + dzeta_dx_s_mg(k,j,i) * dzeta_dx_u_mg(kp1,j,i) *                               &
                            ( p_mg(kp2,j,i-1) + p_mg(kp2,j,i) - p_mg(k,j,i-1) )                           &
                          ...
                          + dzeta_dy_s_mg(k,j,i) * dzeta_dy_v_mg(kp1,j+1,i) *                             &
                            ( p_mg(kp2,j,i) + p_mg(kp2,j+1,i) - p_mg(k,j+1,i) )             )             &
                        - 0.0625_wp * ddzzw_mg_b(k,l) * ddzzw_mg_b(km1,l) * rho_air_mg_b(km1,l) * (             &
                          + dzeta_dx_s_mg(k,j,i) * dzeta_dx_u_mg(km1,j,i) *                               &
                            ( p_mg(k,j,i-1) - p_mg(km2,j,i-1) - p_mg(km2,j,i) )                           &
                          + dzeta_dx_s_mg(k,j,i) * dzeta_dx_u_mg(km1,j,i+1) *                             &
                            ( p_mg(k,j,i+1) - p_mg(km2,j,i) - p_mg(km2,j,i+1) )                           &
                          + dzeta_dy_s_mg(k,j,i) * dzeta_dy_v_mg(km1,j,i) *                               &
                            ( p_mg(k,j-1,i) - p_mg(km2,j-1,i) - p_mg(km2,j,i) )                           &
                          + dzeta_dy_s_mg(k,j,i) * dzeta_dy_v_mg(km1,j+1,i) *                             &
                            ( p_mg(k,j+1,i) - p_mg(km2,j,i) - p_mg(km2,j+1,i) )             )             &
                        + dzeta_dz_s_mg(j,i) * dzeta_dz_s_mg(j,i) * (                                     &
                            ddzzw_mg_b(k,l) * ddzzu_mg_b(kp1,l) * rho_air_zw_mg_b(k,l) * p_mg(kp1,j,i)          &
                          + ddzzw_mg_b(k,l) * ddzzu_mg_b(k,l) * rho_air_zw_mg_b(km1,l) * p_mg(km1,j,i)          &
                                                                    )                                     &
                        ) +                                                                               &
                        ( 2.0_wp * ( ddx2_mg(l) + ddy2_mg(l) ) * rho_air_mg_b(k,l)                          &
                        + 0.0625_wp * ddzzw_mg_b(k,l) * ddzzw_mg_b(kp1,l) * rho_air_mg_b(kp1,l) * (             &
                          ...
                          + dzeta_dy_s_mg(k,j,i) * ( dzeta_dy_v_mg(kp1,j,i) + dzeta_dy_v_mg(kp1,j+1,i) ) )&
                        + 0.0625_wp * ddzzw_mg_b(k,l) * ddzzw_mg_b(km1,l) * rho_air_mg_b(km1,l) * (             &
                            dzeta_dx_s_mg(k,j,i) * ( dzeta_dx_u_mg(km1,j,i) + dzeta_dx_u_mg(km1,j,i+1) )  &
                          + dzeta_dy_s_mg(k,j,i) * ( dzeta_dy_v_mg(km1,j,i) + dzeta_dy_v_mg(km1,j+1,i) ) )&
                        + ddzzw_mg_b(k,l) * ddzzu_mg_b(kp1,l) * rho_air_zw_mg_b(k,l) *                          &
                          ( dzeta_dz_s_mg(j,i) * dzeta_dz_s_mg(j,i) )                                     &
                        + ddzzw_mg_b(k,l) * ddzzu_mg_b(k,l) * rho_air_zw_mg_b(km1,l) *                          &
                          ( dzeta_dz_s_mg(j,i) * dzeta_dz_s_mg(j,i) )                                     &
                        ) * p_mg(k,j,i)
          ENDDO
          !DIR$ IVDEP
          k = nzb+1
          km1 = nzb+1+ind_even_odd
          kp1 = nzb+1+ind_even_odd+1
          km2 = nzb
          kp2 = nzb+2
          r(k,j,i) = f_mg(k,j,i) -                                                                        &
                        ( ddx2_mg(l) * rho_air_mg_b(k,l) * ( p_mg(k,j,i+1) + p_mg(k,j,i-1) )                &
                        ...
                        - 0.0625_wp * ddzzw_mg_b(k,l) * ddzzw_mg_b(km1,l) * rho_air_mg_b(km1,l) * (             &
                          + dzeta_dx_s_mg(k,j,i) * dzeta_dx_u_mg(km1,j,i) *                               &
                            ( p_mg(k,j,i-1) + p_mg(km1,j,i-1) + p_mg(km1,j,i)                             &
                            - 2.0_wp * ( p_mg(km2,j,i-1) + p_mg(km2,j,i) )    )                           &
                          + dzeta_dx_s_mg(k,j,i) * dzeta_dx_u_mg(km1,j,i+1) *                             &
                            ( p_mg(k,j,i+1) + p_mg(km1,j,i) + p_mg(km1,j,i+1)                             &
                            - 2.0_wp * ( p_mg(km2,j,i) + p_mg(km2,j,i+1) )    )                           &
                          + dzeta_dy_s_mg(k,j,i) * dzeta_dy_v_mg(km1,j,i) *                               &
                            ( p_mg(k,j-1,i) + p_mg(km1,j-1,i) + p_mg(km1,j,i)                             &
                            - 2.0_wp * ( p_mg(km2,j-1,i) + p_mg(km2,j,i) )    )                           &
                          + dzeta_dy_s_mg(k,j,i) * dzeta_dy_v_mg(km1,j+1,i) *                             &
                            ( p_mg(k,j+1,i) + p_mg(km1,j,i) + p_mg(km1,j+1,i)                             &
                            - 2.0_wp * ( p_mg(km2,j,i) + p_mg(km2,j+1,i) )    )             )             &
                        + dzeta_dz_s_mg(j,i) * dzeta_dz_s_mg(j,i) * (                                     &
                            ddzzw_mg_b(k,l) * ddzzu_mg_b(kp1,l) * rho_air_zw_mg_b(k,l) * p_mg(kp1,j,i)          &
                          + ddzzw_mg_b(k,l) * ddzzu_mg_b(k,l) * rho_air_zw_mg_b(km1,l) * p_mg(km1,j,i)          &
                                                                    )                                     &
          ...
          ENDDO
          k = ind_even_odd
          km1 = ind_even_odd+ind_even_odd
          kp1 = ind_even_odd+ind_even_odd+1
          km2 = ind_even_odd-1
          r(k,j,i) = f_mg(k,j,i) -                                                                        &
                        ( ddx2_mg(l) * rho_air_mg_b(k,l) * ( p_mg(k,j,i+1) + p_mg(k,j,i-1) )                &
                        + ddy2_mg(l) * rho_air_mg_b(k,l) * ( p_mg(k,j+1,i) + p_mg(k,j-1,i) )                &
                        + 0.25_wp * ddx_mg(l) * ddzzw_mg_b(k,l) * rho_air_zw_mg_b(k,l) * (                    &
                          ...
                          - dzeta_dx_u_mg(k,j,i)   * ( p_mg(kp1,j,i-1) + p_mg(kp1,j,i)                    &
                                                     - p_mg(km1,j,i-1) - p_mg(km1,j,i)   )                &
                          + dzeta_dx_s_mg(k,j,i)   * ( p_mg(k,j,i+1)   - p_mg(k,j,i-1)                    &
                                                     + p_mg(km1,j,i-1) - p_mg(km1,j,i+1) )                &
                                                                                     )                    &
                        + 0.25_wp * ddy_mg(l) * ddzzw_mg_b(k,l) * rho_air_zw_mg_b(k,l) * (                    &
                            dzeta_dy_v_mg(k,j+1,i) * ( p_mg(kp1,j,i)   + p_mg(kp1,j+1,i)                  &
                          ...
                          - dzeta_dy_v_mg(k,j,i)   * ( p_mg(kp1,j-1,i) + p_mg(kp1,j,i)                    &
                                                     - p_mg(km1,j-1,i) - p_mg(km1,j,i)   )                &
                          + dzeta_dy_s_mg(k,j,i)   * ( p_mg(k,j+1,i)   - p_mg(k,j-1,i)                    &
                                                     + p_mg(km1,j-1,i) - p_mg(km1,j+1,i) )                &
                                                                                     )                    &
                        + 0.0625_wp * ddzzw_mg_b(k,l) * ddzzw_mg_b(k,l) * rho_air_zw_mg_b(k,l) * (              &
                          + dzeta_dx_s_mg(k,j,i) * dzeta_dx_u_mg(k,j,i) *                                 &
                            ( p_mg(kp1,j,i-1) + p_mg(kp1,j,i) - p_mg(km1,j,i-1) - p_mg(km1,j,i) )         &
                          + dzeta_dx_s_mg(k,j,i) * dzeta_dx_u_mg(k,j,i+1) *                               &
                            ( p_mg(kp1,j,i) + p_mg(kp1,j,i+1) - p_mg(km1,j,i) - p_mg(km1,j,i+1) )         &
                          + dzeta_dy_s_mg(k,j,i) * dzeta_dy_v_mg(k,j,i) *                                 &
                            ( p_mg(kp1,j-1,i) + p_mg(kp1,j,i) - p_mg(km1,j-1,i) - p_mg(km1,j,i) )         &
                          + dzeta_dy_s_mg(k,j,i) * dzeta_dy_v_mg(k,j+1,i) *                               &
                            ( p_mg(kp1,j,i) + p_mg(kp1,j+1,i) - p_mg(km1,j,i) - p_mg(km1,j+1,i) )         &
                                                                                           )              &
                        - 0.0625_wp * ddzzw_mg_b(k,l) * ddzzw_mg_b(km1,l) * rho_air_mg_b(km1,l) * (             &
                          + dzeta_dx_s_mg(k,j,i) * dzeta_dx_u_mg(km1,j,i) *                               &
                            ( p_mg(k,j,i-1) - p_mg(km2,j,i-1) - p_mg(km2,j,i) )                           &
...
! Description:
! ------------
!> Interpolates the residual on the next coarser grid with "full weighting" scheme 2D version
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE restrict_2d( f_mg, r )
    ...
    REAL(wp), DIMENSION(nys_mg(grid_level+1)-1:nyn_mg(grid_level+1)+1,                             &
                        nxl_mg(grid_level+1)-1:nxr_mg(grid_level+1)+1) ::  r  !< Residual on finer grid level
...
!--       Calculation for the other points along k
          !DIR$ IVDEP
          f_mg(jc,ic) = 1.0_wp / 16.0_wp                                                           &
                              * ( 4.0_wp * r(j,i) + 2.0_wp * ( r(j,i-1)     + r(j,i+1)             &
                                                             + r(j+1,i)     + r(j-1,i)   )         &
                                                  + 1.0_wp * ( r(j-1,i-1)   + r(j+1,i-1)           &
                                                             + r(j-1,i+1)   + r(j+1,i+1) )         &
                                )
       ENDDO
    ENDDO
...
!
!-- Ghost point exchange
    ! since dzeta_dz_s(j,i)'s neighbours are never used in computation, this is not necessary
    !CALL exchange_horiz_2d( f_mg ) 
!
!-- Horizontal boundary conditions
    IF ( .NOT. bc_lr_cyc )  THEN
       IF ( bc_dirichlet_l  .OR.  bc_radiation_l )  THEN
...
!--------------------------------------------------------------------------------------------------!
 ! BOC - Xu
 SUBROUTINE redblack( f_mg, p_mg, dzeta_dx_s_mg, dzeta_dx_u_mg,                                    &
                      dzeta_dy_s_mg, dzeta_dy_v_mg, dzeta_dz_s_mg )
 ! EOC - Xu
    ...
    INTEGER(iwp) ::  km2    !< index variable (k-2)
    INTEGER(iwp) ::  kp2    !< index variable (k+2)
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE sort_k_to_even_odd_blocks_1d( f_mg, f_mg_b, glevel )
    ...
    ! BOC - Xu
    REAL(wp), DIMENSION(nzb:nzt_mg(glevel)+1) ::  f_mg    !< 1D input array
...
!--------------------------------------------------------------------------------------------------!
 ! BOC - Xu
 RECURSIVE SUBROUTINE next_mg_level( f_mg, p_mg, dzeta_dx_s_mg, dzeta_dx_u_mg,                     &
                                     dzeta_dy_s_mg, dzeta_dy_v_mg, dzeta_dz_s_mg, p3, r )
 ! EOC - Xu
    ...
    ! BOC - Xu
    REAL(wp), DIMENSION(nzb:nzt_mg(grid_level-1)+1,nys_mg(grid_level-1)-1:nyn_mg(grid_level-1)+1,  &
                        nxl_mg(grid_level-1)-1:nxr_mg(grid_level-1)+1) ::  dzeta_dx_s_2  !<
    REAL(wp), DIMENSION(nzb:nzt_mg(grid_level-1)+1,nys_mg(grid_level-1)-1:nyn_mg(grid_level-1)+1,  &
                        nxl_mg(grid_level-1)-1:nxr_mg(grid_level-1)+1) ::  dzeta_dx_u_2  !<
    REAL(wp), DIMENSION(nzb:nzt_mg(grid_level-1)+1,nys_mg(grid_level-1)-1:nyn_mg(grid_level-1)+1,  &
                        nxl_mg(grid_level-1)-1:nxr_mg(grid_level-1)+1) ::  dzeta_dy_s_2  !<
    REAL(wp), DIMENSION(nzb:nzt_mg(grid_level-1)+1,nys_mg(grid_level-1)-1:nyn_mg(grid_level-1)+1,  &
                        nxl_mg(grid_level-1)-1:nxr_mg(grid_level-1)+1) ::  dzeta_dy_v_2  !<
    REAL(wp), DIMENSION(nys_mg(grid_level-1)-1:nyn_mg(grid_level-1)+1,                             &
                        nxl_mg(grid_level-1)-1:nxr_mg(grid_level-1)+1) ::  dzeta_dz_s_2  !<
    ! EOC - Xu
...
!--    more accurate solution.
       ...
       ! BOC - Xu
       CALL redblack( f_mg, p_mg, dzeta_dx_s_mg, dzeta_dx_u_mg, dzeta_dy_s_mg, dzeta_dy_v_mg, dzeta_dz_s_mg )
       ! EOC - Xu       
...
!--    Determination of the actual residual
       ! BOC - Xu
       CALL resid( f_mg, p_mg, dzeta_dx_s_mg, dzeta_dx_u_mg, dzeta_dy_s_mg, dzeta_dy_v_mg, dzeta_dz_s_mg, r )
       ! EOC - Xu
...
!--       be used on all PEs after the switch, because then they have the total domain.
       ...
       ELSE
          ...
          ! BOC - Xu
          CALL restrict( dzeta_dx_s_2, dzeta_dx_s_mg )
          CALL restrict( dzeta_dx_u_2, dzeta_dx_u_mg )
          CALL restrict( dzeta_dy_s_2, dzeta_dy_s_mg ) ! why this must be done after CALL restrict( f2, r ) ???
          CALL restrict( dzeta_dy_v_2, dzeta_dy_v_mg )
          CALL restrict_2d( dzeta_dz_s_2, dzeta_dz_s_mg )
          ! EOC - Xu
...
!--    Repeat the same procedure untill the coarsest grid is reached
       ! BOC - Xu
       CALL next_mg_level( f2, p2, dzeta_dx_s_2, dzeta_dx_u_2, dzeta_dy_s_2, dzeta_dy_v_2, dzeta_dz_s_2, p3, r )
       ! EOC - Xu
...
!-- Local variables
    ...
    ALLOCATE( f4_mg_b(nzb:nzt+1,maximum_grid_level), f5_mg_b(nzb:nzt+1,maximum_grid_level) )
    ALLOCATE( f1_mg_b_(nzb:nzt+1,maximum_grid_level), f2_mg_b_(nzb:nzt+1,maximum_grid_level),      &
              f3_mg_b_(nzb:nzt+1,maximum_grid_level) )
    ALLOCATE( f4_mg_b_(nzb:nzt+1,maximum_grid_level), f5_mg_b_(nzb:nzt+1,maximum_grid_level) )
    ALLOCATE( ddzzu_mg_b(nzb:nzt+1,maximum_grid_level), ddzzw_mg_b(nzb:nzt+1,maximum_grid_level) )
    ALLOCATE( rho_air_mg_b(nzb:nzt+1,maximum_grid_level),                                          &
              rho_air_zw_mg_b(nzb:nzt+1,maximum_grid_level) )
    ! EOC - Xu
...
!-- (even/odd) structure
    DO  l = maximum_grid_level, 1 , -1
       CALL sort_k_to_even_odd_blocks( f1_mg(nzb:nzt_mg(grid_level)+1,l),                          &
                                       f1_mg_b(nzb:nzt_mg(grid_level)+1,l), l )
       CALL sort_k_to_even_odd_blocks( f2_mg(nzb:nzt_mg(grid_level)+1,l),                          &
                                       f2_mg_b(nzb:nzt_mg(grid_level)+1,l), l )
       CALL sort_k_to_even_odd_blocks( f3_mg(nzb:nzt_mg(grid_level)+1,l),                          &
                                       f3_mg_b(nzb:nzt_mg(grid_level)+1,l), l )
       ! BOC - Xu
       CALL sort_k_to_even_odd_blocks( f4_mg(nzb:nzt_mg(grid_level)+1,l),                          &
                                       f4_mg_b(nzb:nzt_mg(grid_level)+1,l), l )
       CALL sort_k_to_even_odd_blocks( f5_mg(nzb:nzt_mg(grid_level)+1,l),                          &
                                       f5_mg_b(nzb:nzt_mg(grid_level)+1,l), l )

       CALL sort_k_to_even_odd_blocks( f1_mg_(nzb:nzt_mg(grid_level)+1,l),                         &
                                       f1_mg_b_(nzb:nzt_mg(grid_level)+1,l), l )
       CALL sort_k_to_even_odd_blocks( f2_mg_(nzb:nzt_mg(grid_level)+1,l),                         &
                                       f2_mg_b_(nzb:nzt_mg(grid_level)+1,l), l )
       CALL sort_k_to_even_odd_blocks( f3_mg_(nzb:nzt_mg(grid_level)+1,l),                         &
                                       f3_mg_b_(nzb:nzt_mg(grid_level)+1,l), l )
       CALL sort_k_to_even_odd_blocks( f4_mg_(nzb:nzt_mg(grid_level)+1,l),                         &
                                       f4_mg_b_(nzb:nzt_mg(grid_level)+1,l), l )
       CALL sort_k_to_even_odd_blocks( f5_mg_(nzb:nzt_mg(grid_level)+1,l),                         &
                                       f5_mg_b_(nzb:nzt_mg(grid_level)+1,l), l )

       CALL sort_k_to_even_odd_blocks( ddzzu_mg(nzb:nzt_mg(grid_level)+1,l),                       &
                                       ddzzu_mg_b(nzb:nzt_mg(grid_level)+1,l), l ) ! nzb?
       CALL sort_k_to_even_odd_blocks( ddzzw_mg(nzb:nzt_mg(grid_level)+1,l),                       & 
                                       ddzzw_mg_b(nzb:nzt_mg(grid_level)+1,l), l )
       CALL sort_k_to_even_odd_blocks( rho_air_mg(nzb:nzt_mg(grid_level)+1,l),                     & 
                                       rho_air_mg_b(nzb:nzt_mg(grid_level)+1,l), l )
       CALL sort_k_to_even_odd_blocks( rho_air_zw_mg(nzb:nzt_mg(grid_level)+1,l),                  &
                                       rho_air_zw_mg_b(nzb:nzt_mg(grid_level)+1,l), l )
       ! EOC - Xu
    ENDDO
