...
!--          Tendency terms for potential temperature
             ...
             ENDIF
             CALL diffusion_s( i, j, pt, surf_def_h(0)%shf, surf_def_h(1)%shf, surf_def_h(2)%shf,  &
...
!--          Prognostic equation for potential temperature
             DO  k = nzb+1, nzt
                ! BOC - DEBUG
                !IF ( k==nzb+1 .AND. j==12 .AND. i==12 ) THEN
                !   WRITE(*,*) "tend_pt: ", tend(k,j,i)
                !ENDIF
                ! EOC - DEBUG
                pt_p(k,j,i) = pt(k,j,i) +                                                          &
...
!
!--       Calculate metrics
          CALL module_interface_actions( i, j, 'after_prognostic_equations' )
       ENDDO
    ENDDO

    CALL module_interface_exchange_horiz( 'after_computing_metrics' )
    ! EOC - Xu
...
!--    pt-tendency terms with no communication
       ...
       CALL diffusion_s( pt, surf_def_h(0)%shf, surf_def_h(1)%shf, surf_def_h(2)%shf,              &
