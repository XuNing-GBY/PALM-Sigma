    ...
    DO  i = nxl, nxr
       DO  j = nys, nyn
          ...
          d(k,j,i) = ( ( u(k,j,i+1) - u(k,j,i) ) * rho_air(k) * ddx +                              &
                       ...
                       - ( u(k-1,j,i+1) + u(k-1,j,i) ) * rho_air(k-1) * 2.0_wp                     &
                       ...
                       - ( v(k-1,j+1,i) + v(k-1,j,i) ) * rho_air(k-1) * 2.0_wp                     &
                       ) * 0.25_wp * ddzzw(k) * dzeta_dy_s(k,j,i) +                                &
                       ( w(k,j,i)   * rho_air_zw(k) - w(k-1,j,i) * rho_air_zw(k-1) )               &
                       * ddzzw(k) * dzeta_dz_s(j,i) ) * ddt_3d * d_weight_pres
          DO k = nzb+2, nzt
             d(k,j,i) = ( ( u(k,j,i+1) - u(k,j,i) ) * rho_air(k) * ddx +                           &
                          ( ( u(k+1,j,i+1) + u(k+1,j,i) ) * rho_air(k+1)                           &
                          - ( u(k-1,j,i+1) + u(k-1,j,i) ) * rho_air(k-1)                           &
...
!--    Solve Poisson equation for perturbation pressure using SOR-Red/Black scheme
       ...
       ! BOC - Xu
       CALL sor( d, ddzzu_pres, ddzzw, p_loc )
       ! EOC - Xu
       tend = p_loc
...
!-- imposed at the boundaries.
    ...
    DO  i = nxl, nxr
       DO  j = nys, nyn
          DO  k = nzb+1, nzt
             !IF ( k == nzt-1 .AND. j==4 .AND. i==4 ) THEN
             !   write(*,*) "w: ", w(k,j,i)
             !   write(*,*) "w+: ", - dt_3d * ( tend(k+1,j,i) - tend(k,j,i) ) * ddzzu(k+1) * dzeta_dz_s(j,i) * weight_pres_l
             !ENDIF
             w(k,j,i) = w(k,j,i) - dt_3d * ( tend(k+1,j,i) - tend(k,j,i) ) * ddzzu(k+1)            &
                        * dzeta_dz_s(j,i)                                                          &
                        * weight_pres_l                                                            &
                        * MERGE( 1.0_wp, 0.0_wp, BTEST( topo_flags(k,j,i), 3 ) )
          ENDDO
          ...
          u(k,j,i) = u(k,j,i) - dt_3d * ( ( tend(k,j,i) - tend(k,j,i-1) ) * ddx                    &
                                        + ( tend(k+1,j,i-1) + tend(k+1,j,i)                        &
                                          + tend(k,j,i-1)   + tend(k,j,i)                          &
                                          - 2.0_wp * ( tend(k-1,j,i-1) + tend(k-1,j,i) )           &
          ...
          v(k,j,i) = v(k,j,i) - dt_3d * ( ( tend(k,j,i) - tend(k,j-1,i) ) * ddy                    &
                                        + ( tend(k+1,j-1,i) + tend(k+1,j,i)                        &
                                          + tend(k,j-1,i)   + tend(k,j,i)                          &
                                          - 2.0_wp * ( tend(k-1,j-1,i) + tend(k-1,j,i) )           &
