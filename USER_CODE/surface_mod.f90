...
!--             Check if current gridpoint belongs to the atmosphere
                IF ( BTEST( topo_flags(k,j,i), 0 ) )  THEN
                   IF ( .NOT. BTEST( topo_flags(k+bc_h(l)%koff, j+bc_h(l)%joff,                    &
...
!--             Check if current gridpoint belongs to the atmosphere
                IF ( BTEST( topo_flags(k,j,i), 0 ) )  THEN
                   IF ( .NOT. BTEST( topo_flags(k+bc_v(l)%koff, j+bc_v(l)%joff,                    &
...
!--                   test the adjacent grid cell
                      IF ( .NOT. BTEST( topo_flags(kk,j,i), 0 ) )  THEN
!
!--                      Determine flags indicating a terrain surface, a building surface,
                         terrain  = BTEST( topo_flags(kk,j,i), 5 )  .OR.  topo_no_distinct
                         building = BTEST( topo_flags(kk,j,i), 6 )  .OR.  topo_no_distinct
!
...
!--                      surfaces.
                         unresolved_building = BTEST( topo_flags(kk,j,i), 5 )  .AND.               &
                                               BTEST( topo_flags(kk,j,i), 6 )
!
!--                      Land-surface type
...
!
!--                      Unclassifified surface-grid point. Give error message. Note, in previous
!--                      versions the error message was generalized using a combination of TRIM
!--                      and MERGE function. However, due to compile errors with the Cray compiler
!--                      two seperate error messages are given now.
                         ELSE
                            IF ( kk==0 )  THEN
                               WRITE( message_string, * ) 'Unclassified upward ',                  &
                                        '-facing surface element at grid point (k,j,i) = ', k, j, i
                               CALL message( 'surface_mod', 'PA0698', 1, 2, myid, 6, 0 )
                            ELSE
                               WRITE( message_string, * ) 'Unclassified downward ',                &
                                        '-facing surface element at grid point (k,j,i) = ', k, j, i
                               CALL message( 'surface_mod', 'PA0698', 1, 2, myid, 6, 0 )
                            ENDIF
                         ENDIF
...
!
!--             Northward-facing
                IF ( .NOT. BTEST( topo_flags(k,j-1,i), 0 ) )  THEN
!
!--                Determine flags indicating terrain or building

                   terrain  = BTEST( topo_flags(k,j-1,i), 5 )  .OR.  topo_no_distinct
                   building = BTEST( topo_flags(k,j-1,i), 6 )  .OR.  topo_no_distinct

                   unresolved_building = BTEST( topo_flags(k,j-1,i), 5 )  .AND.                    &
                                         BTEST( topo_flags(k,j-1,i), 6 )
...
!
!--             Eastward-facing
                IF ( .NOT. BTEST( topo_flags(k,j,i-1), 0 ) )  THEN
!
!--                Determine flags indicating terrain or building
                   terrain  = BTEST( topo_flags(k,j,i-1), 5 )  .OR.  topo_no_distinct
                   building = BTEST( topo_flags(k,j,i-1), 6 )  .OR.  topo_no_distinct

                   unresolved_building = BTEST( topo_flags(k,j,i-1), 5 )  .AND.                    &
                                         BTEST( topo_flags(k,j,i-1), 6 )
...
!-- Initialize surface-layer height
    ...
    IF ( upward_facing )  THEN
       surf%z_mo(num_h) = ( zzu(k) - zzw(k-1) ) * H_av
    ELSE
       surf%z_mo(num_h) = ( zzw(k) - zzu(k)   ) * H_av
    ENDIF
    ! EOC - Xu
...
!--    All data writen with rd_mpi_io_write_surface are globally indexed 1d-arrays.
       ...
       DO  l = 0, 2
          ...
          ns_h_on_file(l) = total_number_of_surface_elements
