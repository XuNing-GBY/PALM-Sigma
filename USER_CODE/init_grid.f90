 ...
 SUBROUTINE init_grid
    ...
    USE arrays_3d,                                                                                 &
        ONLY:  dd2zu, ddzu, ddzu_pres, ddzw, dzu, dzw, x, xu, y, yv, zu, zw,                       &
               dd2zzu, ddzzu, ddzzu_pres, ddzzw, dzzu, dzzw, zzu, zzw
    ! EOC - Xu
    ...
    ! BOC - Xu
    USE grid_variables,                                                                            &
        ONLY:  ddx, ddx2, ddy, ddy2, dx, dx2, dy, dy2, zu_s_inner, zw_w_inner, H_av
    ! EOC - Xu
...
!-- Allocate grid arrays
    ...
    ! BOC - Xu
    ALLOCATE( ddzzu(1:nzt+1) )
    ALLOCATE( ddzzw(1:nzt+1) )
    ALLOCATE( dd2zzu(1:nzt) )
    ALLOCATE( dzzu(1:nzt+1) )
    ALLOCATE( dzzw(1:nzt+1) )
    ALLOCATE( zzu(nzb:nzt+1) )
    ALLOCATE( zzw(nzb:nzt+1) )
    ! EOC - Xu
...
!-- Compute grid lengths.
    ...
    DO k = 1, nzt+1
       dzzu(k)  = zzu(k) - zzu(k-1)
       ddzzu(k) = 1.0_wp / dzzu(k)
       dzzw(k)  = zzw(k) - zzw(k-1)
       ddzzw(k) = 1.0_wp / dzzw(k)
    ENDDO

    DO k = 1, nzt
       dd2zzu(k) = 1.0_wp / ( dzzu(k) + dzzu(k+1) )
    ENDDO
    ! EOC - Xu
...
!-- additional array containing with appropriate grid information is created for these solvers.
    IF ( psolver(1:9) /= 'multigrid' )  THEN
       ...
       ! BOC - Xu
       ALLOCATE( ddzzu_pres(1:nzt+1) )
       ddzzu_pres = ddzzu
       ddzzu_pres(1) = ddzzu_pres(2)
       ! EOC - Xu
    ENDIF
...
!--          scalar grid
             IF ( BTEST( topo(k,j,i), 0 ) )                                                        &
                topo_flags(k,j,i) = IBSET( topo_flags(k,j,i), 0 )
!
!--          u grid
...
!--       Special flag to control vertical diffusion at model top - former nzt_diff
          ...
          IF ( use_top_fluxes )                                                &
             topo_flags(nzt+1,j,i) = IBCLR( topo_flags(nzt+1,j,i), 9 )
...
!-- Uppermost topography index on scalar grid
    ibit = 12
    topo_top_ind(:,:,0) = MAXLOC( MERGE( 1, 0, BTEST( topo_flags(:,:,:), ibit ) ), DIM=1 )         &
                          - 1
!
!-- Uppermost topography index on u grid
    ibit = 14
    topo_top_ind(:,:,1) = MAXLOC( MERGE( 1, 0, BTEST( topo_flags(:,:,:), ibit ) ), DIM=1 )         &
                          - 1
!
!-- Uppermost topography index on v grid
    ibit = 16
    topo_top_ind(:,:,2) = MAXLOC( MERGE( 1, 0, BTEST( topo_flags(:,:,:), ibit ) ), DIM=1 )         &
                          - 1
!
!-- Uppermost topography index on w grid
    ibit = 18
    topo_top_ind(:,:,3) = MAXLOC( MERGE( 1, 0, BTEST( topo_flags(:,:,:), ibit ) ), DIM=1 )         &
                          - 1
!
!-- Uppermost topography index on scalar outer grid
    ibit = 24
    topo_top_ind(:,:,4) = MAXLOC( MERGE( 1, 0, BTEST( topo_flags(:,:,:), ibit ) ), DIM=1 )         &
                          - 1
!
