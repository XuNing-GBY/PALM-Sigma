...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE init_3d_model
    ...
    ! BOC - Xu
    USE grid_variables,                                                                            &
        ONLY:  dx, dy, ddx_mg, ddy_mg, ddx2_mg, ddy2_mg, H_av
    ! EOC - Xu
    ...
    LOGICAL ::  nesting_flag !< control flag indicating that the model run does not belong to a child domain or
                             !< does not use vertical nesting
...
!-- purposes. This density does not need to be considered in the model's system of equations.
    ...
    ELSE
       DO  k = nzb, nzt+1
          ! BOC - Xu
          p_hydrostatic(k) = barometric_formula(zzu(nzb)*H_av, pt_surface *                        &
                                                exner_function(surface_pressure * 100.0_wp),       &
                                                surface_pressure * 100.0_wp)
          ! EOC - Xu
...
!-- respective density on each grid.
    IF ( psolver(1:9) == 'multigrid' )  THEN
       ...
       ! BOC - Xu
       ALLOCATE( ddx_mg(maximum_grid_level) )
       ALLOCATE( ddy_mg(maximum_grid_level) )
       ...
       ALLOCATE( dzzu_mg(nzb+1:nzt+1,maximum_grid_level) )
       ALLOCATE( dzzw_mg(nzb+1:nzt+1,maximum_grid_level) )
       ALLOCATE( ddzzu_mg(nzb+1:nzt+1,maximum_grid_level) )
       ALLOCATE( ddzzw_mg(nzb+1:nzt+1,maximum_grid_level) )
       ...
       ALLOCATE( f4_mg(nzb+1:nzt,maximum_grid_level) )
       ALLOCATE( f5_mg(nzb+1:nzt,maximum_grid_level) )

       ALLOCATE( f1_mg_(nzb+1:nzt,maximum_grid_level) )
       ALLOCATE( f2_mg_(nzb+1:nzt,maximum_grid_level) )
       ALLOCATE( f3_mg_(nzb+1:nzt,maximum_grid_level) )
       ALLOCATE( f4_mg_(nzb+1:nzt,maximum_grid_level) )
       ALLOCATE( f5_mg_(nzb+1:nzt,maximum_grid_level) )

       ALLOCATE( rho_air_mg(nzb:nzt+1,maximum_grid_level) )
       ALLOCATE( rho_air_zw_mg(nzb:nzt+1,maximum_grid_level) )

       ALLOCATE( zzu_mg(nzb:nzt+1,maximum_grid_level) )
       ALLOCATE( zzw_mg(nzb:nzt+1,maximum_grid_level) )

       dzu_mg(:,maximum_grid_level) = dzu
       dzzu_mg(:,maximum_grid_level) = dzzu
       DO k = 1, nzt+1 
          ddzzu_mg(k,maximum_grid_level) = 1.0_wp/dzzu_mg(k,maximum_grid_level)
       ENDDO

       rho_air_mg(:,maximum_grid_level) = rho_air
       zzu_mg(:,maximum_grid_level) = zzu
!
!--    Next line to ensure an equally spaced grid.
       dzu_mg(1,maximum_grid_level) = dzu(2)
       dzzu_mg(1,maximum_grid_level) = dzzu(2)

       rho_air_mg(nzb,maximum_grid_level) = rho_air(nzb) + (rho_air(nzb) - rho_air(nzb+1))

       dzw_mg(:,maximum_grid_level) = dzw
       dzzw_mg(:,maximum_grid_level) = dzzw
       DO k = 1, nzt+1
          ddzzw_mg(k,maximum_grid_level) = 1.0_wp/dzzw_mg(k,maximum_grid_level)
       ENDDO

       rho_air_zw_mg(:,maximum_grid_level) = rho_air_zw

       zzw_mg(:,maximum_grid_level) = zzw

       nzt_l = nzt
       DO  l = maximum_grid_level-1, 1, -1
           dzu_mg(nzb+1,l) = 2.0_wp * dzu_mg(nzb+1,l+1)
           dzzu_mg(nzb+1,l) = 2.0_wp * dzzu_mg(nzb+1,l+1)
           ddzzu_mg(nzb+1,l) = 1.0_wp/dzzu_mg(nzb+1,l)
           dzw_mg(nzb+1,l) = 2.0_wp * dzw_mg(nzb+1,l+1)
           dzzw_mg(nzb+1,l) = 2.0_wp * dzzw_mg(nzb+1,l+1)
           ddzzw_mg(nzb+1,l) = 1.0_wp/dzzw_mg(nzb+1,l)
           ...
           zzu_mg(nzb,l) = zzu(nzb)
           zzw_mg(nzb,l) = zzw(nzb)
           zzu_mg(nzb+1,l) = zzu(nzb+1)
           zzw_mg(nzb+1,l) = zzw(nzb+1)

           nzt_l = nzt_l / 2
           DO  k = 2, nzt_l+1
              dzu_mg(k,l) = dzu_mg(2*k-2,l+1) + dzu_mg(2*k-1,l+1)
              dzzu_mg(k,l) = dzzu_mg(2*k-2,l+1) + dzzu_mg(2*k-1,l+1)
              ddzzu_mg(k,l) = 1.0_wp/dzzu_mg(k,l)
              dzw_mg(k,l) = dzw_mg(2*k-2,l+1) + dzw_mg(2*k-1,l+1)
              dzzw_mg(k,l) = dzzw_mg(2*k-2,l+1) + dzzw_mg(2*k-1,l+1)
              ddzzw_mg(k,l) = 1.0_wp/dzzw_mg(k,l)
              rho_air_mg(k,l)    = rho_air_mg(2*k-1,l+1)
              rho_air_zw_mg(k,l) = rho_air_zw_mg(2*k-1,l+1)

              zzu_mg(k,l) = zzu_mg(2*k-1,l+1)
              zzw_mg(k,l) = zzw_mg(2*k-1,l+1)              
           ENDDO
       ENDDO
       ...
       DO  l = maximum_grid_level, 1, -1
          ddx_mg(l)  = 1.0_wp / dx_l
          ddy_mg(l)  = 1.0_wp / dy_l
          ...
          DO  k = nzb+1, nzt_l
             ...
             f4_mg(k,l) = rho_air_zw_mg(k,l) * 0.5_wp / dzu_mg(k+1,l)
             f5_mg(k,l) = rho_air_zw_mg(k-1,l) * 0.5_wp / dzu_mg(k,l)

             f2_mg_(k,l) = rho_air_zw_mg(k,l) / ( dzzu_mg(k+1,l) * dzzw_mg(k,l) )
             f3_mg_(k,l) = rho_air_zw_mg(k-1,l) / ( dzzu_mg(k,l) * dzzw_mg(k,l) )
             f1_mg_(k,l) = 2.0_wp * ( ddx2_mg(l) + ddy2_mg(l) )                                     &
                          * rho_air_mg(k,l) + f2_mg_(k,l) + f3_mg_(k,l)
             f4_mg_(k,l) = rho_air_zw_mg(k,l) * 0.5_wp / dzzu_mg(k+1,l)
             f5_mg_(k,l) = rho_air_zw_mg(k-1,l) * 0.5_wp / dzzu_mg(k,l)             
          ENDDO
          nzt_l = nzt_l / 2
          dx_l  = dx_l * 2.0_wp
          dy_l  = dy_l * 2.0_wp
...
!--          Default for the transition range: one tenth of the undamped layer
          ...
          DO  k = nzb, nzt+1
             ...
             ! BOC - Xu
             IF ( zzu(k) * H_av <= inflow_damping_height )  THEN
                inflow_damping_factor(k) = 1.0_wp
             ELSEIF ( zzu(k) * H_av <= ( inflow_damping_height + inflow_damping_width ) )  THEN
                inflow_damping_factor(k) = 1.0_wp -                                                &
                                           ( zzu(k) * H_av - inflow_damping_height ) / inflow_damping_width
             ELSE
...
!--          index arrays with their respective offset values.
             DO  l = 0, 3
                CALL init_single_surface_properties( surf_def_v(l)%qsws, tmp_2d%var,               &
                                                     surf_def_v(l)%ns, tmp_2d%fill,                &
...
!--          Read variable
             ...
             CALL exchange_horiz_2d( tmp_2d%var )
!
!--          Initialize passive scalar flux. Initialize horizontal surface elements.
             CALL init_single_surface_properties( surf_def_h(0)%ssws, tmp_2d%var, surf_def_h(0)%ns,&
                                                  tmp_2d%fill, surf_def_h(0)%i, surf_def_h(0)%j )

             DO  l = 0, 3
                CALL init_single_surface_properties( surf_def_v(l)%ssws, tmp_2d%var,               &
                                                     surf_def_v(l)%ns, tmp_2d%fill,                &
                                                     surf_def_v(l)%i+surf_def_v(l)%ioff,           &
                                                     surf_def_v(l)%j+surf_def_v(l)%joff )
             ENDDO
...
!--             In this case, use the lowest surface-level height.
                IF ( surf_def_h(0)%start_index(j,i) <= surf_def_h(0)%end_index(j,i) )  THEN
                   ...
                   k = surf_def_h(0)%k(m)
                   mean_surface_level_height_l(sr) = mean_surface_level_height_l(sr) + zzw(k-1) * H_av
                ENDIF
...
!-- removal in case of bottom and top Neumann boundary conditions (not in nested setups) before
!-- the pressure solver is invoked. Note, the removal will not be done for offline or online nested
!-- simulations.
!-- To check for the nesting status, store the logical on a temporary varialbe. This is necessary
!-- to compile the code also without MPI (nesting mode is only available in parallel mode).
    nesting_flag = .TRUE.
#if defined( __parallel )
    nesting_flag =  .NOT. ( child_domain .AND. nesting_mode /= 'vertical' )
#endif
...
#if defined( __parallel )
       IF ( collective_wait )  CALL MPI_BARRIER( comm2d, ierr )
       CALL MPI_ALLREDUCE( MPI_IN_PLACE, ngp_2dh_wgrid(1), nzt-nzb+1, MPI_INTEGER, MPI_SUM, comm2d, ierr )
#endif
!
...
!-- external NetCDF file.
    ...
    ! BOC - Xu
    CALL module_interface_init    ! not sure if it is ok to call this in advance ???
    ! EOC - Xu
!
...
!--    Check temperature in case of too large domain height
       ! BOC - Xu
       DO  k = nzb, nzt+1
          IF ( ( pt_surface * exner_function( surface_pressure * 100.0_wp )                        &
               - g/c_p * zzu(k) * H_av ) < 0.0_wp )  THEN
