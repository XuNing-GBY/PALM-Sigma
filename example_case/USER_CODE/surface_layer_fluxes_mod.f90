...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE surface_layer_fluxes
    ...
    ! BOC - Xu
    IF ( TRIM( wwinta_mode ) == 'rgh' ) CALL change_roughness
    ! EOC - Xu
...
!--       To mask this, use ibit, which checks for upward/downward-facing surfaces.
          ! BOC - Xu
          IF ( TRIM( wwinta_mode ) == 'sfc' .AND. .NOT. downward ) THEN
             IF ( les_dynamic ) THEN
                surf%uvw_abs(m) = SQRT( ( 0.5_wp * ( u(k,j,i) + u(k,j,i+1) -                        &
                                                     spv_u(j,i) - spv_u(j,i+1) ) * sia_cos_x(j,i)     &
                                        + 0.5_wp * ( w(k,j,i) + w(k-1,j,i) -                        &
                                                     spv_w(j,i) * 2.0_wp       ) * sia_sin_x(j,i)     &
                                        )**2                                                          &
                                      + ( 0.5_wp * ( v(k,j,i) + v(k,j+1,i) -                        &
                                                     spv_v(j,i) - spv_v(j+1,i) ) * sia_cos_y(j,i)     &
                                        + 0.5_wp * ( w(k,j,i) + w(k-1,j,i) -                        &
                                                     spv_w(j,i) * 2.0_wp       ) * sia_sin_y(j,i)     &
                                        )**2                                                          &
                                      )
             ELSE
                surf%uvw_abs(m) = SQRT( ( 0.5_wp * ( u(k,j,i) + u(k,j,i+1) -                          &
       ...
       IF ( .NOT. downward )  THEN
          ...
          DO  m = 1, surf%ns
             ...
             IF ( TRIM( wwinta_mode ) == 'sfc' ) THEN
                IF ( les_dynamic ) THEN
                   surf%usws(m) = kappa * ( ( u(k,j,i) - spv_u(j,i) ) *                               &
                                            ( sia_cos_x(j,i) + sia_cos_x(j,i-1) ) * 0.5_wp +          &
                                            ( ( w(k,j,i)   + w(k,j,i-1) +                             &
                                                w(k-1,j,i) + w(k-1,j,i-1) ) * 0.25_wp -               &
                                              ( spv_w(j,i) + spv_w(j,i-1) ) * 0.5_wp ) *              &
                                            ( sia_sin_x(j,i) + sia_sin_x(j,i-1) ) * 0.5_wp            &
                                          )                                                           &
...
!--    Upward-facing surfaces
       IF ( .NOT. downward )  THEN
          ...
          DO  m = 1, surf%ns
             ...
             IF ( TRIM( wwinta_mode ) == 'sfc' ) THEN
                IF ( les_dynamic ) THEN
                   surf%vsws(m) = kappa * ( ( v(k,j,i) - spv_v(j,i) ) *                              &
                                            ( sia_cos_y(j,i) + sia_cos_y(j-1,i) ) * 0.5_wp +          &
                                            ( ( w(k,j,i)   + w(k,j-1,i) +                           &
                                                w(k-1,j,i) + w(k-1,j-1,i) ) * 0.25_wp -             &
                                              ( spv_w(j,i) + spv_w(j-1,i) ) * 0.5_wp ) *              &
                                            ( sia_sin_y(j,i) + sia_sin_y(j-1,i) ) * 0.5_wp            &
                                          )                                                           &
                                  / ( surf%ln_z_z0(m) - psi_m( z_mo / surf%ol(m) )                    &
                                                      + psi_m( surf%z0(m) / surf%ol(m) ) )
                ELSE
