...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE flow_statistics
    ...
    ! BOC - Xu
    USE arrays_3d,                                                                                 &
        ONLY:  ddzu, ddzzu, ddzw, ddzzw, d_exner, e, exner,                                        &
               heatflux_output_conversion, hyp, km, kh,                                            &
               ...
               waterflux_output_conversion, zw, zzw
    ! EOC - Xu
...
!--             Momentum flux w"u"
                ...
                sums_l(k,12,tn) = sums_l(k,12,tn) - 0.25_wp * (                                    &
                                  km(k,j,i) + km(k+1,j,i) + km(k,j,i-1) + km(k+1,j,i-1)            &
                                      ...
                                      ( u(k+1,j,i) - u(k,j,i)   ) * ddzzu(k+1) * dzeta_dz_u(j,i)   &
...
!--             Momentum flux w"v"
                ...
                sums_l(k,14,tn) = sums_l(k,14,tn) - 0.25_wp * (                                    &
                                  km(k,j,i) + km(k+1,j,i) + km(k,j-1,i) + km(k+1,j-1,i)            &
                                      ...
                                      ( v(k+1,j,i) - v(k,j,i)   ) * ddzzu(k+1) * dzeta_dz_v(j,i)   &
                                    + ( w(k,j,i)   - w(k,j-1,i) ) * ddy                            &
                                    + ( w(k+1,j,i) + w(k+1,j-1,i)                                  &
                                      - w(k-1,j,i) - w(k-1,j-1,i)                                  &
                                      ) * 0.25_wp * ddzzu(k+1)                                     &
                                        * ( dzeta_dy_w(k,j,i) + dzeta_dy_w(k,j-1,i) ) * 0.5_wp     &
                                                                  ) * rmask(j,i,sr)                &
                                            * rho_air_zw(k)                                        &
                                            * momentumflux_output_conversion(k)                    &
                                            * flag
...
!--             Heat flux w"pt"
                ...
                sums_l(k,16,tn) = sums_l(k,16,tn)                                                  &
                                         - 0.5_wp * ( kh(k,j,i) + kh(k+1,j,i) )                    &
                                               ...
                                               * ddzzu(k+1) * dzeta_dz_s(j,i) * rmask(j,i,sr)     &
                                               * flag
                      ...
                      DO  m = surf_lsm_h(l)%start_index(j,i),                                      &
                         ...
                         IF ( humidity )  THEN
                            ...
                            sums_l(k+ki,45,tn) = sums_l(k+ki,45,tn) + (                            &
                                                ( 1.0_wp + 0.61_wp * q(k+ki,j,i) )                 &
                                                * surf_lsm_h(l)%shf(m) +                           &
                                                  0.61_wp * pt(k+ki,j,i) * surf_lsm_h(l)%qsws(m) ) &
                                                * heatflux_output_conversion(k+ki)
                            IF ( cloud_droplets )  THEN
                               sums_l(k+ki,45,tn) = sums_l(k+ki,45,tn) + (                         &
                                                   ( 1.0_wp + 0.61_wp * q(k+ki,j,i) -              &
                                                     ql(k+ki,j,i) ) * surf_lsm_h(l)%shf(m) +       &
                                                     0.61_wp * pt(k+ki,j,i)                        &
                                                   * surf_lsm_h(l)%qsws(m) )                       &
                                                   * heatflux_output_conversion(k+ki)
                            ENDIF
                   ...
                   IF ( surf_usm_h(l)%ns >= 1 )  THEN
                      DO  m = surf_usm_h(l)%start_index(j,i),                                      &
...
#if 0
                         sums_l(k+ki,58,tn) = sums_l(k+ki,58,tn) + 0.0_wp * rmask(j,i,sr)  ! u"pt"
                         sums_l(k+ki,61,tn) = sums_l(k+ki,61,tn) + 0.0_wp * rmask(j,i,sr)  ! v"pt"
#endif
#ifndef _OPENACC
                         IF ( ocean_mode )  THEN
                            sums_l(k+ki,65,tn) = sums_l(k+ki,65,tn) +                              &
                         ...
                         IF ( humidity )  THEN
                            ...
                            sums_l(k+ki,45,tn) = sums_l(k+ki,45,tn) + (                            &
                                                ( 1.0_wp + 0.61_wp * q(k+ki,j,i) ) *               &
                                                surf_usm_h(l)%shf(m) + 0.61_wp * pt(k+ki,j,i) *    &
                                                surf_usm_h(l)%qsws(m)  )                           &
                                                * heatflux_output_conversion(k+ki)
                            IF ( cloud_droplets )  THEN
                               sums_l(k+ki,45,tn) = sums_l(k+ki,45,tn) + (                         &
                                                   ( 1.0_wp + 0.61_wp * q(k+ki,j,i) -              &
                                                    ql(k+ki,j,i) ) * surf_usm_h(l)%shf(m) +        &
                                                    0.61_wp * pt(k+ki,j,i) * surf_usm_h(l)%qsws(m) )&
                                                   * heatflux_output_conversion(k+ki)
                            ENDIF
                            IF ( bulk_cloud_model )  THEN
!
...
!--    added on top of LSM sums.
       IF ( land_surface  .AND.  urban_surface )  THEN
          ...
          DO  m = 1, surf_usm_h(0)%ns
             ...
             sums_l(nzb,93,tn)  = sums_l(nzb,93,tn) + surf_usm_h(0)%wghf_eb(m) * rmask(j,i,sr)
             sums_l(nzb,97,tn)  = sums_l(nzb,97,tn) + surf_usm_h(0)%r_a(m)     * rmask(j,i,sr)
          ENDDO
          !$OMP END PARALLEL
       ENDIF
!
...
!--    Calculation is time consuming. Do it only, if profiles shall be plotted.
       IF ( hom(nzb+1,2,55,0) /= 0.0_wp  .OR.  hom(nzb+1,2,68,0) /= 0.0_wp )  THEN
          ...
          DO  i = nxl, nxr
             DO  j = nys, nyn
                DO  k = nzb+1, nzt
                   sums_l(k,55,tn) = ( sums_ll(k,1) - sums_ll(k-1,1) ) * ddzzw(k) * dzeta_dz_s(j,i)
                   sums_l(k,56,tn) = ( sums_ll(k,2) - sums_ll(k-1,2) ) * ddzzw(k) * dzeta_dz_s(j,i)
             ...
             DO  j = nys, nyn
                DO  k = nzb+1, nzt
                   ...
                   sums_l(k,57,tn) = sums_l(k,57,tn) - 0.5_wp * (                                  &
                                       (km(k,j,i)+km(k+1,j,i)) * (e(k+1,j,i)-e(k,j,i)) * ddzzu(k+1)&
                                                               * dzeta_dz_s(j,i)                  &
                                     - (km(k-1,j,i)+km(k,j,i)) * (e(k,j,i)-e(k-1,j,i)) * ddzzu(k)  &
                                                               * dzeta_dz_s(j,i)                  &
                                                                ) * ddzzw(k) * dzeta_dz_s(j,i)    &
                                                                  * flag * rmask(j,i,sr)

                   sums_l(k,69,tn) = sums_l(k,69,tn) - 0.5_wp * (                                  &
                                        ( km(k,j,i) + km(k+1,j,i) ) *                              &
                                        ( e(k+1,j,i) - e(k,j,i) ) * ddzzu(k+1) * dzeta_dz_s(j,i)  &
                                                                ) * flag * rmask(j,i,sr)
                   ! EOC - Xu
                ENDDO
             ENDDO
...
!--       Average land-surface quantities over LSM surfaces only
          IF ( land_surface  .AND.  surf_lsm_h(0)%ns_tot > 0 )  THEN
             sums(k,89:92) = sums(k,89:92) / surf_lsm_h(0)%ns_tot
             sums(k,94:96) = sums(k,94:96) / surf_lsm_h(0)%ns_tot
             sums(k,98)    = sums(k,98)    / surf_lsm_h(0)%ns_tot
          ENDIF
!
...
!--       This needs to be revised in the future.
          IF ( land_surface  .AND.  surf_lsm_h(0)%ns_tot + surf_usm_h(0)%ns_tot > 0 )  THEN
           !  print*, surf_lsm_h(0)%ns_tot + surf_usm_h(0)%ns_tot, surf_lsm_h(0)%ns_tot, surf_usm_h(0)%ns_tot
             sums(k,93) = sums(k,93) / ( surf_lsm_h(0)%ns_tot + surf_usm_h(0)%ns_tot )
             sums(k,97) = sums(k,97) / ( surf_lsm_h(0)%ns_tot + surf_usm_h(0)%ns_tot )
          ENDIF
          ...
          IF ( ocean_mode )  THEN
             DO  k = nzt, nzb+1, -1
                ...
                IF ( hom(k,1,18,sr) < -1.0E-8_wp  .AND.  hom(k-1,1,18,sr) > hom(k,1,18,sr) )  THEN
                   IF ( zzw(k) * H_av < 1.5_wp * height )  THEN
                      z_i(1) = zzw(k) * H_av
                   ELSE
                      z_i(1) = height
                   ENDIF
...
!--       ocean case!
          ...
          z_i(2) = 0.0_wp
          DO  k = nzb+1, nzt+1
             dptdz(k) = ( hom(k,1,4,sr) - hom(k-1,1,4,sr) ) * ddzzu(k) * dzeta_dz_s(j,i)
          ENDDO
