...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE advec_u_pw_ij( i, j )
    ...
    USE wind_wave_inta_mod,                                                                        &
        ONLY:  dzeta_dt_u, dzeta_dx_u, dzeta_dy_u, dzeta_dz_u
    ! EOC - Xu
    ...
    DO  k = nzb+1, nzt
       IF ( k == nzb+1 ) THEN
          tend(k,j,i) = tend(k,j,i) - dzeta_dt_u(k,j,i) * ( u(k+1,j,i) - u(k-1,j,i) ) * dd2zzu(k)  &
                      - 0.25_wp * (                                                                &
                        ( u(k,j,i+1) * ( u(k,j,i+1) + u(k,j,i) - gu )                              &
                        - u(k,j,i-1) * ( u(k,j,i-1) + u(k,j,i) - gu ) ) * ddx                      &
                      + ( u(k+1,j,i) * ( u(k+1,j,i) + u(k,j,i) - gu )                              &
                        - u(k-1,j,i) * ( u(k,j,i) + u(k-1,j,i) - gu ) )                            &
                        * ddzzw(k) * dzeta_dx_u(k,j,i) * 1.33333333333_wp                          &
                      + ( u(k,j+1,i) * ( v(k,j+1,i-1) + v(k,j+1,i) - gv )                          &
                        - u(k,j-1,i) * ( v(k,j,i-1)   + v(k,j,i)   - gv ) ) * ddy                  &
                      + ( u(k+1,j,i) * ( v(k+1,j,i) + v(k+1,j,i-1) + v(k+1,j+1,i) + v(k+1,j+1,i-1) &
                                       + v(k,j,i)   + v(k,j,i-1)   + v(k,j+1,i)   + v(k,j+1,i-1)   &
                                       - 4.0_wp * gv ) * 0.25_wp                                   &
                        - u(k-1,j,i) * ( v(k-1,j,i) + v(k-1,j,i-1) + v(k-1,j+1,i) + v(k-1,j+1,i-1) &
                                       + v(k,j,i)   + v(k,j,i-1)   + v(k,j+1,i)   + v(k,j+1,i-1)   &
                                       - 4.0_wp * gv ) * 0.25_wp )                                 &
                        * ddzzw(k) * dzeta_dy_u(k,j,i) * 1.33333333333_wp                          &
                      + ( u(k+1,j,i) * ( w(k,j,i) + w(k,j,i-1) )                                   &
                        - u(k-1,j,i) * ( 0.75_wp * ( w(k-1,j,i) + w(k-1,j,i-1) )                   &
                                       + 0.25_wp * ( w(k,j,i)   + w(k,j,i-1)   ) ) )               &
                        * ddzzw(k) * dzeta_dz_u(j,i) * 1.33333333333_wp                            &
                                  )
       ELSE
          tend(k,j,i) = tend(k,j,i) - dzeta_dt_u(k,j,i) * ( u(k+1,j,i) - u(k-1,k,i) ) * dd2zzu(k)  &
                      - 0.25_wp * (                                                                &
                        ( u(k,j,i+1) * ( u(k,j,i+1) + u(k,j,i) - gu )                              &
                        - u(k,j,i-1) * ( u(k,j,i-1) + u(k,j,i) - gu ) ) * ddx                      &
                      + ( u(k+1,j,i) * ( u(k+1,j,i) + u(k,j,i) - gu )                              &
