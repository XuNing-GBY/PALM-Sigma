...
!--    If required, compute initial profile of the geostrophic wind (component ug)
       ...
       IF ( .NOT. ocean_mode )  THEN
          ...
          DO  k = 1, nzt+1
             IF ( i < 11 )  THEN
                IF ( ug_vertical_gradient_level(i) < zzu(k) * H_av  .AND.                          &
             ...
             IF ( gradient /= 0.0_wp )  THEN
                IF ( k /= 1 )  THEN
                   ug(k) = ug(k-1) + dzzu(k) * H_av * gradient
                ELSE
                   ug(k) = ug_surface + dzzu(k) * H_av * gradient
                ENDIF
             ELSE
                ug(k) = ug(k-1)
             ENDIF
       ...
       ELSE
          ...
          DO  k = nzt, nzb, -1
             ...
             IF ( gradient /= 0.0_wp )  THEN
                IF ( k /= nzt )  THEN
                   ug(k) = ug(k+1) - dzzu(k+1) * H_av * gradient
                ELSE
                   ug(k)   = ug_surface - 0.5_wp * dzzu(k+1) * H_av * gradient
                   ug(k+1) = ug_surface + 0.5_wp * dzzu(k+1) * H_av * gradient
                ENDIF
             ELSE
                ug(k) = ug(k+1)
             ENDIF
...
!--    If required, compute initial profile of the geostrophic wind (component vg)
       ...
       IF ( .NOT. ocean_mode )  THEN
          ...
          DO  k = 1, nzt+1
             IF ( i < 11 )  THEN
                IF ( vg_vertical_gradient_level(i) < zzu(k) * H_av  .AND.                          &
             ...
             IF ( gradient /= 0.0_wp )  THEN
                IF ( k /= 1 )  THEN
                   vg(k) = vg(k-1) + dzzu(k) * H_av * gradient
                ELSE
                   vg(k) = vg_surface + dzzu(k) * H_av * gradient
                ENDIF
             ELSE
                vg(k) = vg(k-1)
             ENDIF
       ...
       ELSE
          ...
          DO  k = nzt, nzb, -1
             ...
             IF ( gradient /= 0.0_wp )  THEN
                IF ( k /= nzt )  THEN
                   vg(k) = vg(k+1) - dzzu(k+1) * H_av * gradient
                ELSE
                   vg(k)   = vg_surface - 0.5_wp * dzzu(k+1) * H_av * gradient
                   vg(k+1) = vg_surface + 0.5_wp * dzzu(k+1) * H_av * gradient
                ENDIF
             ELSE
                vg(k) = vg(k+1)
             ENDIF
...
!--       Non-cyclic boundary conditions are realized in the pressure solver only for specific FFTs
    ...
    IF ( XOR( bc_lr_cyc, bc_ns_cyc )  .AND.  psolver(1:9) /= 'multigrid' )  THEN
