...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE advec_w_pw_ij( i, j )
    ...
    USE wind_wave_inta_mod,                                                                        &
        ONLY:  dzeta_dt_w, dzeta_dx_w, dzeta_dy_w, dzeta_dz_s
    ! EOC - Xu
    ...
    DO  k = nzb+1, nzt
       tend(k,j,i) = tend(k,j,i)                                                                   &
                 - dzeta_dt_w(k,j,i) * ( w(k+1,j,i) - w(k-1,j,i) ) * ( ddzzw(k+1) + ddzzw(k) )     &
                 - 0.25_wp * (                                                                     &
                   ( w(k,j,i+1) * ( u(k+1,j,i+1) + u(k,j,i+1) - gu )                               &
                   - w(k,j,i-1) * ( u(k+1,j,i)   + u(k,j,i)   - gu ) ) * ddx                       &
                 + ( w(k+1,j,i) * ( u(k+1,j,i+1) + u(k+1,j,i) - gu )                               &
                   - w(k-1,j,i) * ( u(k,j,i+1)   + u(k,j,i)   - gu ) )                             &
                   * ddzzu(k+1) * dzeta_dx_w(k,j,i)                                                &
                 + ( w(k,j+1,i) * ( v(k+1,j+1,i) + v(k,j+1,i) - gv )                               &
                   - w(k,j-1,i) * ( v(k+1,j,i)   + v(k,j,i)   - gv ) ) * ddy                       &
                 + ( w(k+1,j,i) * ( v(k+1,j+1,i) + v(k+1,j,i) - gv )                               &
                   - w(k-1,j,i) * ( v(k,j+1,i)   + v(k,j,i)   - gv ) )                             &
