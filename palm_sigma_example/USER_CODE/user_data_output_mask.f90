...
!--             Default masked output
                DO  i = 1, mask_size_l(mid,1)
                   DO  j = 1, mask_size_l(mid,2)
                      DO  k = 1, mask_size_l(mid,3)
                         local_pf(i,j,k) = wu_sgs(mask_k(mid,k),                                   &
                                                  mask_j(mid,j),                                   &
                                                  mask_i(mid,i))
                      ENDDO
                   ENDDO
...
!--                   Save output array
                      DO  k = 1, mask_size_l(mid,3)
                         local_pf(i,j,k) = wu_sgs(MIN( topo_top_index + mask_k(mid,k), nzt+1 ),    &
                                                     mask_j(mid,j), mask_i(mid,i) )
                      ENDDO
                   ENDDO
                ENDDO
...
!--             Default masked output
                DO  i = 1, mask_size_l(mid,1)
                   DO  j = 1, mask_size_l(mid,2)
                      DO  k = 1, mask_size_l(mid,3)
                          local_pf(i,j,k) = wu_sgs_av(mask_k(mid,k), mask_j(mid,j), mask_i(mid,i) )
                       ENDDO
                    ENDDO
                 ENDDO
             ELSE
...
!--                   Save output array
                      DO  k = 1, mask_size_l(mid,3)
                         local_pf(i,j,k) = wu_sgs_av( MIN( topo_top_index+mask_k(mid,k), nzt+1 ),   &
                                                     mask_j(mid,j), mask_i(mid,i) )
                      ENDDO
                   ENDDO
                ENDDO
...
!--             Default masked output
                DO  i = 1, mask_size_l(mid,1)
                   DO  j = 1, mask_size_l(mid,2)
                      DO  k = 1, mask_size_l(mid,3)
                         local_pf(i,j,k) = wv_sgs(mask_k(mid,k),                                   &
                                                  mask_j(mid,j),                                   &
                                                  mask_i(mid,i))
                      ENDDO
                   ENDDO
...
!--                   Save output array
                      DO  k = 1, mask_size_l(mid,3)
                         local_pf(i,j,k) = wv_sgs(MIN( topo_top_index + mask_k(mid,k), nzt+1 ),    &
                                                     mask_j(mid,j), mask_i(mid,i) )
                      ENDDO
                   ENDDO
                ENDDO
...
!--             Default masked output
                DO  i = 1, mask_size_l(mid,1)
                   DO  j = 1, mask_size_l(mid,2)
                      DO  k = 1, mask_size_l(mid,3)
                          local_pf(i,j,k) = wv_sgs_av(mask_k(mid,k), mask_j(mid,j), mask_i(mid,i) )
                       ENDDO
                    ENDDO
                 ENDDO
             ELSE
...
!--                   Save output array
                      DO  k = 1, mask_size_l(mid,3)
                         local_pf(i,j,k) = wv_sgs_av( MIN( topo_top_index+mask_k(mid,k), nzt+1 ),   &
                                                     mask_j(mid,j), mask_i(mid,i) )
                      ENDDO
                   ENDDO
                ENDDO
...
!--             Default masked output
                DO  i = 1, mask_size_l(mid,1)
                   DO  j = 1, mask_size_l(mid,2)
                      DO  k = 1, mask_size_l(mid,3)
                         local_pf(i,j,k) = wpt_sgs(mask_k(mid,k),                                 &
                                                   mask_j(mid,j),                                 &
                                                   mask_i(mid,i))
                      ENDDO
                   ENDDO
...
!--                   Save output array
                      DO  k = 1, mask_size_l(mid,3)
                         local_pf(i,j,k) = wpt_sgs(MIN( topo_top_index + mask_k(mid,k), nzt+1 ),  &
                                                      mask_j(mid,j), mask_i(mid,i) )
                      ENDDO
                   ENDDO
                ENDDO
...
!--             Default masked output
                DO  i = 1, mask_size_l(mid,1)
                   DO  j = 1, mask_size_l(mid,2)
                      DO  k = 1, mask_size_l(mid,3)
                          local_pf(i,j,k) = wpt_sgs_av(mask_k(mid,k), mask_j(mid,j), mask_i(mid,i) )
                       ENDDO
                    ENDDO
                 ENDDO
             ELSE
...
!--                   Save output array
                      DO  k = 1, mask_size_l(mid,3)
                         local_pf(i,j,k) = wpt_sgs_av( MIN( topo_top_index+mask_k(mid,k), nzt+1 ), &
                                                      mask_j(mid,j), mask_i(mid,i) )
                      ENDDO
                   ENDDO
                ENDDO
